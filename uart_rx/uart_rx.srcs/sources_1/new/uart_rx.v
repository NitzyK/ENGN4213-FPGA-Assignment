module uart_rx (
    ...
    output reg [7:0] data,   // Received parallel data
    output reg data_valid    // High when data is valid
);


parameter CLKS_PER_BIT = 10416;      // Actual value: 10416.666
...
// Internal registers
reg [3:0] bit_index;       // Tracks which bit we're receiving (0-7)
reg [15:0] clk_counter;    // Counts clock cycles for baud timing
...
// -------------------------------------------
// State Transition Logic (next state only)
// -------------------------------------------
always @(posedge clk or posedge reset) begin
    if (reset) begin
        current_state <= IDLE;
